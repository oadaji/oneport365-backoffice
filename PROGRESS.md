...
### Task 19: Phase 3 — Architectural cleanup (dedup, server-side sender, hash caching, consolidation)
- **3.1:** Deduplicated `extractForwardedSender` from gmail.ts and email-sync.ts into `src/lib/forwarded-sender.ts`. Unit test with 5 forward formats (all passing).
- **3.2:** Moved `effectiveSender` to server as `resolveSender()` in `src/lib/resolve-sender.ts`. Implements full 4-step priority chain from EMAIL_DETAIL_SPEC.md (fromEmail → extracted Email field → extracted Customer/Company → fallback). Stores `resolvedSenderName`/`resolvedSenderEmail` on Rfq model. Called from both ingestion paths and re-extract. Frontend `effectiveSender()` replaced with `getSender()` that reads server-computed fields. Backfill script at `src/scripts/backfill-resolved-sender.ts` — run manually: `npx ts-node -r dotenv/config src/scripts/backfill-resolved-sender.ts`
- **3.3:** Content-hash re-extract: added `lastExtractionHash` to Rfq model. SHA-256 of normalized thread body (whitespace-collapsed, quoted-reply markers stripped). Re-extract skips Claude call if hash unchanged, saving API cost and latency.
- **3.4:** Verified no inline `require()` calls remain in rfqs.ts (the nodemailer require was replaced in 3.6).
